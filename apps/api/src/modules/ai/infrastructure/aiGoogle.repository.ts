...
const CONTACT_INFO_SYSTEM_INSTRUCTION =
...
export class AiGoogleRepository extends AiRepository {
  ...
  async extractContactInfo(content: string): Promise<ContactInfo> {
    ...
    try {
      const structuredModel = this.model.withStructuredOutput(ContactInfoSchema);
      return await structuredModel.invoke([new SystemMessage(CONTACT_INFO_SYSTEM_INSTRUCTION), new HumanMessage(userMessage)]);
    } catch (error) {
